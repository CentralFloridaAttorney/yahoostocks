...
symbols, names = np.array(sorted(stocks.items())).T
quotes = []
#
...
for symbol in stocks:
    yahoostock = YahooStocks(symbol)
    quotes.append(yahoostock.price_frame)

close_prices = np.vstack([q["close"][-10:] for q in quotes])
open_prices = np.vstack([q["open"][-10:] for q in quotes])
