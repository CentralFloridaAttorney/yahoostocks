...
class DBTool:
    """
    Initializes a new instance of the DBTool class, which provides access to MySQL functions within Python.
    """
    ...
    def __init__(
        ...
        :param table_name: (optional) The name of the table to use.
        :param config_key: (optional) The name of the configuration section to use from the default.ini file. Defaults to 'bilbo'.
        :param user_name: (optional) The username for the MySQL database. If not provided, the value is read from the configuration file.
        :param password: (optional) The password for the MySQL database. If not provided, the value is read from the configuration file.
        :param host: (optional) The host name for the MySQL database. If not provided, the value is read from the configuration file.
        :param port: (optional) The port number for the MySQL database. If not provided, the value is read from the configuration file.
        :param onehotdb_name: (optional) The name of the onehot database. If not provided, the value is read from the configuration file.
        :param onehotdb_table: (optional) The name of the onehot table. If not provided, the value is read from the configuration file.
        """
        # Set up logging
        self.logger = logging.getLogger("DBTool")
        self.logger.setLevel(logging.DEBUG)
        log_formatter = logging.Formatter("%(asctime)s - %(levelname)s - %(message)s")
        log_handler = logging.handlers.RotatingFileHandler(
            "dbtool.log", maxBytes=1024 * 1024, backupCount=5)
        ...
        configtool = ConfigTool(config_key)
        these_configs = configtool.get_configs()
        self.user = user_name or these_configs.get('user')
        self.password = password or these_configs.get('passwd')
        self.host = host or these_configs.get('host')
        self.port = port or these_configs.get('port')
        self.database_name = database_name or these_configs.get('database_name')
        self.table_name = table_name or these_configs.get('table_name')
        self.onehotdb_name = onehotdb_name or these_configs.get('onehotdb_name')
        self.onehotdb_table = onehotdb_table or these_configs.get('onehotdb_table')
        self.open_database(self.database_name)
        self.logger.debug("DBTool.__init__ config loaded successfully: " + self.user)
    ...
    def _get_db_connection(self, host_name, user_name, user_password, port_num, _db_name=None):
        ...
        try:
            ...
            if err.errno == 1049:
                ...
                self.logger.error('*** Did you update default.ini: ' + self.user)
                # print('created new database: ' + db_name)
        return connection

    def _get_db_connection_bak(self, host_name, user_name, user_password, port_num, _db_name=None):
        connection = None
        try:
            if _db_name is not None:
                connection = mysql.connector.connect(
    ...
    def _execute_mysql(self, _mysql_statement, _value=None):
        ...
        """
        result = 'default'
        connection = self._get_db_connection(self.host, self.user, self.password, self.port, self.database_name)
        if connection is not None:
            try:
                ...
                if err.errno == 1054 or str(err.args[1]).endswith('exists') or str(err.args[1]).__contains__(
                        'Duplicate'):
                    self.logger.warning(f"***** non-fatal error in dbtool._execute_mysql\nError: '{err}'\n" + _mysql_statement)
                elif err.errno == 1064:
                    self.logger.error(f"***** mysql syntax error\nError: '{err}'\n" + _mysql_statement)
    ...
    def _execute_mysql_bak(self, _mysql_statement, _value=None):
        """
        DBTool._execute_mysql() executes MySQL commands, which are called by other methods.
        :param _mysql_statement: a formatted string with a MySQL command.
        :param _value: RESERVED for future use
    ...
    def delete_link_key(self, _link_key):
        pass
    ...
    def get_dataframe(self):
        ...
        if not all_rows:
            self.logger.warning(f"No rows found in table {self.table_name}")
            return None
        dataframe = pandas.DataFrame(all_rows)
        # some results do not need to be transposed
        # dataframe = dataframe.transpose()
        # dataframe = dataframe.replace([None], "default")
        columns = self.get_columns(_exclude_2_keys=False)
        dataframe.columns = columns
        return dataframe

    def get_dataframe_bak(self):
        get_all_rows_mysql_statement = MYSQL_SELECT_FROM.format('*', self.table_name)
        all_rows = self._execute_mysql(get_all_rows_mysql_statement)
        dataframe = pandas.DataFrame(all_rows)
        columns = self.get_columns(_exclude_2_keys=False)
...
class ConfigTool:
    ...
    def write_default_configs(self):
        file_path = f'{self.config_key}.ini'
        config_object = ConfigParser(defaults=DEFAULT_INI)
